...
ALTER TABLE notifications
    ...
    -- When the provider confirmed the handset had it.
    --
    -- `sent_at` is a different moment: it is when *we* handed the message
    -- over, stamped at acceptance. So `sent_at IS NOT NULL` means "we sent
    -- it", and only `delivered_at IS NOT NULL` means "it arrived". Reading
    -- sent_at as delivery is the conflation this migration exists to undo.
